...
import { MOCK_FULL_DATES, MOCK_SEPTEMBER_AVAILABILITY } from '../../pages/user/appointments/bookingAstrologerData.js'
...
function formatTime(value, durationMinutes = 30) {
  const [clock, meridiem] = value.split(' ')
  const [hours, minutes] = clock.split(':').map(Number)
  const end = new Date(2026, 0, 1, (hours % 12) + (meridiem === 'PM' ? 12 : 0), minutes + durationMinutes)
  return `${value} – ${end.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}`
}

function slotDurationMinutes(value) {
...
export default function AppointmentBookingModal({ astrologer, availability = {}, appointments = [], userWallet, userId, userName, actions, routes, onClose, initialDate = '', initialSelectedSlots = [], pricePerSlot = PRICE, slotDuration = '30 Minutes', initialStep = 'form', initialDetails = null }) {
  ...
  const [month, setMonth] = useState(() => initialDate ? new Date(parseKey(initialDate).getFullYear(), parseKey(initialDate).getMonth(), 1) : (firstDate ? new Date(parseKey(firstDate).getFullYear(), parseKey(firstDate).getMonth(), 1) : new Date(2026, 7, 1)))
  const [date, setDate] = useState(initialDate || firstDate)
  ...
  const chooseSlot = (nextDate, nextTime) => {
    ...
    setSelectedSlots((current) => alreadySelected
      ...
      : [...current, { key: slotKey, date: nextDate, time: nextTime, duration: slotDuration, type: TYPE, package: '30 Min Consultation', price: pricePerSlot }])
    setDate(nextDate)
    setTime(nextTime)
  }
        ...
        <div className="modal-card__content user-modal-card__content">
          {step === 'form' && <>
            <div className="availability-calendar appointment-calendar-extended">
              <div className="availability-calendar__header"><button type="button" className="icon-btn" aria-label="Previous month" onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() - 1, 1))}><ChevronLeft size={16} /></button><strong>{month.toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</strong><div className="appointment-calendar-nav"><button type="button" className="icon-btn" aria-label="Next month" onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() + 1, 1))}><ChevronRight size={16} /></button></div></div>
