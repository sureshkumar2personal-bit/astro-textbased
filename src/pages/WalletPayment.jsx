...
export default function WalletPayment() {
  ...
  const confirmPayment = () => { if (!sufficient || processing || paid) return; setProcessing(true); actions.debitUserWallet({ amount: totalPayable, astrologer: astrologer.name, duration: selectedPackage.duration, service: 'Call', transactionId: paymentKey }); localStorage.setItem(paymentKey, 'successful'); setPaid(true); setConfirming(false); setProcessing(false) }
